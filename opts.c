...
static void calc(struct opts_t *opts);
...
struct opts_t get_opts(int argc, char **argv) {
    ...
    // address wasn't given
    if (!o.addr)
        usage(argv[0], 1);

    // calculate number of nonces, stagger_size, nonces_per_thread, etc.
    calc(&o);
...
static void calc(struct opts_t *o) {
    ...
    o->nonces_per_thread = (unsigned long)(o->stagger_size / o->num_threads);
    if (o->nonces_per_thread == 0) {
        o->num_threads = o->stagger_size;
        o->nonces_per_thread = 1;
    }
